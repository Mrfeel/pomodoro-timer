...
$$\oint_L \boldsymbol{H}\cdot d\boldsymbol{l} = I_f + \cancelto{0}{\int_{S_1}\frac{\partial\boldsymbol{D}}{\partial t}\cdot d\boldsymbol{S}} = I$$

（导线中 $\partial\boldsymbol{D}/\partial t$ 通常可忽略，传导电流主导）

## 4.2 对环路 $L$，选面 $S_2$（穿过电容缝隙）

$$\oint_L \boldsymbol{H}\cdot d\boldsymbol{l} = \cancelto{0}{I_f} + \int_{S_2}\frac{\partial\boldsymbol{D}}{\partial t}\cdot d\boldsymbol{S} = I_d = I$$
...
with open(r"d:\辰辰\first CC\ampere_maxwell.md", "w", encoding="utf-8") as f:
    f.write(content)
